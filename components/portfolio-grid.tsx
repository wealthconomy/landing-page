...
import { Lock, Users, Target, Wallet, Heart, Repeat, ArrowUpRight, ArrowRight, type LucideIcon } from "lucide-react";
...
const accentMap: Record<string, { iconBg: string; badgeBg: string; glow: string }> = {
  flex: { 
    ...
    badgeBg: "bg-red-500/5 text-red-600 dark:text-red-400 border border-red-500/10",
    glow: "bg-red-500/10", 
  },
  fix: { 
    iconBg: "bg-amber-500/10 text-amber-600 dark:text-amber-500 border border-amber-500/10", 
    badgeBg: "bg-amber-500/5 text-amber-700 dark:text-amber-400 border border-amber-500/10",
    glow: "bg-amber-500/10", 
  },
  goal: { 
    iconBg: "bg-pink-500/10 text-pink-500 border border-pink-500/10", 
    badgeBg: "bg-pink-500/5 text-pink-600 dark:text-pink-400 border border-pink-500/10",
    glow: "bg-pink-500/10", 
  },
  fam: { 
    iconBg: "bg-violet-500/10 text-violet-500 border border-violet-500/10", 
    badgeBg: "bg-violet-500/5 text-violet-600 dark:text-violet-400 border border-violet-500/10",
    glow: "bg-violet-500/10", 
  },
  flow: { 
    iconBg: "bg-sky-500/10 text-sky-500 border border-sky-500/10", 
    badgeBg: "bg-sky-500/5 text-sky-600 dark:text-sky-400 border border-sky-500/10",
    glow: "bg-sky-500/10", 
  },
  group: { 
    iconBg: "bg-emerald-500/10 text-emerald-500 border border-emerald-500/10", 
    badgeBg: "bg-emerald-500/5 text-emerald-600 dark:text-emerald-400 border border-emerald-500/10",
...
export function PortfolioGrid() {
  return (
    <section id="portfolio" className="bg-background py-10 lg:py-14 border-b border-border/40 relative overflow-hidden">
          ...
          {products.map((p) => {
            ...
            return (
              ...
              >
                ...
                <div className={cn("absolute -right-16 -top-16 h-36 w-36 rounded-full blur-2xl opacity-40 transition-opacity duration-500 group-hover:opacity-60 pointer-events-none", pa.glow)} />
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="relative flex items-center justify-center h-12 w-12">
                      ...
                      <div className={cn("absolute inset-0 rounded-full border border-dashed animate-spin-slow pointer-events-none transition-colors duration-500", 
                        p.accent === "flex" ? "border-red-500/40" :
                        p.accent === "fix" ? "border-amber-500/40" :
                        p.accent === "goal" ? "border-pink-500/40" :
                        p.accent === "fam" ? "border-violet-500/40" :
                        p.accent === "flow" ? "border-sky-500/40" :
                        "border-emerald-500/40"
                      )} />
                      {/* Inner blur glow */}
                      <div className={cn("absolute inset-1 rounded-full blur-sm opacity-0 transition-opacity duration-500 group-hover:opacity-100", pa.glow)} />
                      {/* Colored icon container */}
                      <div className={cn("relative flex h-8 w-8 items-center justify-center rounded-lg transition-all duration-500 group-hover:scale-105 group-hover:rotate-3 z-10", pa.iconBg)}>
                    ...
                    <span className="relative flex h-1.5 w-1.5 mr-1">
                      <span className={cn("animate-ping absolute inline-flex h-full w-full rounded-full opacity-75", dotMap[p.accent])}></span>
                      <span className={cn("relative inline-flex rounded-full h-1.5 w-1.5", dotMap[p.accent])}></span>
                    </span>
                  </div>

                  {/* Info */}
                  <div className="mt-3">
                    <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-[8.5px] font-bold uppercase tracking-wider mb-2", pa.badgeBg)}>
                      {p.bestFor}
                    </span>
